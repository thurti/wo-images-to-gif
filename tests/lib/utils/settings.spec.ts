...
describe("lib/utils", () => {
  describe("#getSettingsString()", () => {
    ...
    it("should check if setting scale has correct value", () => {
      const settings = {
        filter_complex: {
          id: "filter_complex",
          label: "Filter",
          value: "-filter_complex scale={scale}[s]",
        },
        scale: { id: "scale", label: "Width", value: "123" },
      };

      const settingsString = getSettingsString(settings);

      expect(settingsString).toContain("123:-1");
    });
  });
  ...
  describe("#createFFmegCommandArgs()", () => {
    ...
    it("should use filename params if present", () => {
      const format = {
        id: "mp3",
        label: "mp3",
        value: "mp3",
