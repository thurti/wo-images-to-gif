...
describe("lib/utils", () => {
  ...
  describe("#getFilenamesWithIndex()", () => {
    it("should return filenames with index", () => {
      const files = [
        new File([""], "test1.txt"),
        new File([""], "test2.txt"),
        new File([""], "test3.txt"),
      ];
      const filenames = getIndexedFilenames(files);
      expect(filenames).toEqual(["test1_1.txt", "test1_2.txt", "test1_3.txt"]);
    });

    it("should return empty array if no files are specified", () => {
      const filenames = getIndexedFilenames([]);
