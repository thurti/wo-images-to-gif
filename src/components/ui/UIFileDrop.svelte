  ...
  const onDropFiles = (event: DragEvent) => {
    ...
    const droppedFiles = Array.from(event.dataTransfer?.files ?? [])
      .map((file) => {
        ...
        while (files.findIndex((f) => f?.name === newFilename) > -1) {
          newFilename = `${file.name}_${counter++}`;
        }
...
<div
  class="file-drop theme-border-color-50 relative flex max-w-sm select-none items-center justify-center rounded-lg border-2 border-dashed bg-opacity-25 p-8 pb-4 transition-colors w-full"
...
<style>
  ...
  .file-drop label.reduced {
    @apply p-0 w-7 h-7 text-2xl font-thin;
  }
